...
class Pedometer:
    ...
    def add(self, ax, ay, az):
        l1 = filt.l1_norm(ax, ay, az) # Calculate L1-norm
        ...
        self.__l1.add(l1) # Add L1-norm to the CircularList
        self.__new_samples += num_add # Increment the new sample count

    """
    Process the new data to update step count
    """
    def process(self):
        # # Only process if enough new samples are available
        # min_samples = 13  # or the padlen required by filter
        # if self.__new_samples < min_samples:
        #     return self.__steps, [], np.array(self.__filtered)
